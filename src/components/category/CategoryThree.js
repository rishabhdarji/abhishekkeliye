...
const CategoryThree = ( { wrapperClass, styleClass } ) => {
    return (
        ...
        <div className={ `row ${ wrapperClass || 'g-5 mt--25' }` }>
            <div style={{fontSize:"40px",fontWeight:"bolder",color:"black"}}> Admission Requirements</div>
            <div>Here are the major requirements to study in UK which you need to ensure while applying to a UK university:</div>
            { items.map( ( data , i ) => (
                <ScrollAnimation 
                    animateIn="fadeInUp"
